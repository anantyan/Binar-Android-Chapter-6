...
class ProfileFragment : Fragment(), EasyPermissions.PermissionCallbacks {
    ...
    private fun onBindView() {
        ...
        binding.btnSetProfile.setOnClickListener {
            val idUsr = requireContext().preference2().getUserId()
            viewModel.getAccount(idUsr)
        }
        binding.fabSetPassword.setOnClickListener {
            val idUsr = requireContext().preference2().getUserId()
            requireContext().dialog().dialogSetPassword(idUsr) { item, dialog ->
            ...
            when (resultCode) {
                RESULT_OK -> {
                    data?.let { intent ->
                        ...
                        val idUsr = requireContext().preference2().getUserId()
                        viewModel.setPhoto(idUsr, requireContext().buildImageBodyPart("image", bitmap))
                    }
                }
